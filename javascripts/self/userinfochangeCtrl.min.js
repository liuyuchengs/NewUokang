	...
	$scope.change = function(){
		if($scope.check()){
			...
			.success(function(data){
				...
				Tool.comfirm($scope,"数据更新失败!");
			})
		}
	}
	$scope.check = function(){
		if($scope.val.length==0){
			Tool.comfirm($scope,"填写内容为空！");
			return false;
		}else if($scope.item=="realname"|$scope.item=="nickname"){
			if($scope.val.length>2&$scope.val.length<10){
				return true;
			}else{
				Tool.comfirm($scope,"请输入正确长度的名称！");
				return false;
			}
			...
			if(/^1[3|4|5|7|8]\d{9}$/.test($scope.val)){
				...
				Tool.comfirm($scope,"请输入正确的手机号码!");
				return false;
			}
		}else if($scope.item=="age"){
			if(parseInt($scope.val)&parseInt($scope.val)>0&parseInt($scope.val)<150){
				return true;
			}else{
				Tool.comfirm($scope,"请输入正确的年龄!");
				return false;
			}
		}else{
			Tool.comfirm($scope,"未识别需要修改的项目,请稍后再试!");
			return false;
		}
	}
