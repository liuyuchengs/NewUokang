app.controller("grabCtrl",["$scope","$http","$window","Tool","Ajax",function($scope,$http,$window,Tool,Ajax){
	$scope.queryParams = {
		dayDate:"",
		profressionId:2,
		city:"深圳",
	...
	$scope.classSelect = function(params){
		...
		$scope.queryParams.profressionId = $scope.classParams[params].id;
		$scope.queryParams.currentPage = 1;
		$scope.grabs=[];
		$scope.loadData();
	...
	$scope.loadData = function(){
		$scope.loading = true;
		var url = Tool.getSession("host")+"/wx/gift/queryproduct";
			...
			if(Tool.isLogin()){
				if(Tool.getLocal("user")){
					...
					if(user.giftCode==null||user.giftCode==0){
						Tool.alert($scope,"一个用户只能享受一次免费项目，您已经抢过了!");
					}else{
						Tool.goPage("/new/htmls/grab-code.html#?productId="+productId+"&hospitalId="+hospitalId+"&date="+$scope.queryParams.dayDate);
					}
				}
			}else{
	...
	$scope.question = function(){
		Tool.alert($scope,"每个用户只可享受一次免费抢单，如需帮助请致电：0755-26905699")
	}
}])
