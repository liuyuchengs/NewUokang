app.controller("orderCtrl",["$scope","$http","Tool",function($scope,$http,Tool){
	$scope.orders;
	$scope.cancelOrderId;
	$scope.userId = Tool.getLocal("user").id;
	$scope.noOrder = false;
	$scope.btnParams = {
		toPay:false,
		payed:false,
	...
	//加载订单数据
	$scope.load = function(){
		$scope.loadOrder($scope.userId);
	}

	//加载订单数据,---还需要添加分类查询---
	$scope.loadOrder = function(userId){
		var Id = ""
		if(userId){
			Id = userId;
		}
		...
		.success(function(data){
			if(data.code==0){
				...
				Tool.alert($scope,"获取订单信息失败，请稍后再试!");
			}
		}).error(function(){
			Tool.alert($scope,"获取订单信息失败，请稍后再试!");
		})
	...
	$scope.toPay = function(order){
		...
		var redirect_uri = Tool.getSession("host")+"/new/html/weixin_pay.html";
