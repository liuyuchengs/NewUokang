app.controller("registerCtrl",["$scope","$http","$interval","Tool","Ajax",function($scope,$http,$interval,Tool,Ajax){
	...
	*/
	$scope.register = function(){
		if($scope.codeCheck()&$scope.check()){
	...
	$scope.phoneCheck = function(){
		if(/^1[3|4|5|7|8]\d{9}$/.test($scope.phone)){
			...
			$http.post(url,data,{
				...
				$scope.message = "手机号码检查失败，请稍后再试!";
				return false;
			})
		}else{
			$scope.message = "手机号码不正确！";
			return false;
		}
	}
	...
	$scope.sendSmsCode = function(){
		...
		$http.post(url,data,{
			...
			if(data.code==0){
				...
				$scope.message = data.mesage;
				return false;
			}
		}).error(function(){
			$scope.message = "发送验证码失败，请稍后再试!";
			return false;
		})
	}
