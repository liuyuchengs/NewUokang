app.controller("homeCtrl",["$scope","$http","Tool","Ajax","Weixin",function($scope,$http,Tool,Ajax,Weixin){
	...
	$scope.load = function(){
		...
		$scope.getLocation(latitude,longitude,$scope.key);
	}
	...
	$scope.getLocation = function(latitude,longitude){
		...
		$http.jsonp(url).success(function(data){
			if(data.status==0){
				$scope.locationInfo.city = data.result.address_component.city;
			}else{
				$scope.locationInfo.city = "定位失败";
			}
		}).error(function(){
			$scope.locationInfo.city = "定位失败";
		})
	...
	*/
	$scope.queryBanner = function(){
		var url = Tool.getSession("host")+"/wx/banner/query";
