...
app.service("Weixin",["$http","Tool",function($http,Tool){
	...
	this.wxCheckPayParams = function(params){
		if(empty(params.timestamp)|empty(params.nonceStr)|empty(params.package)|empty(params.signType)|empty(params.paySign)){
			return false;
		} else {
			return true;
		}
	...
	this.wxPay = function(orderId,code,scope){
		var params = {
			...
			paySign: '',
			success:function() {
				Tool.setSession("payResult","success");
				Tool.goPage("/new/htmls/pay-result.html");
			},
			cancel:function(){
				Tool.setSession("payResult","fail");
				Tool.goPage("/new/htmls/pay-result.html");
			},
			fail:function(){
		...
		if(orderId!=""&orderId!=null){
			var accessToken = Tool.getSession("accessToken");
			...
			$http.post(url,params,{
				...
				if(this.wxCheckPayParams(params)){
