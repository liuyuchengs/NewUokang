app.controller("detailCtrl",["$scope","$http","$location","Tool",function($scope,$http,$location,Tool){
	...
	*/
	$scope.queryProductImg = function(){
		var url = Tool.getSession("host")+"/wx/image/querybymainid?type=PRODUCT&mainId="+$scope.proId;
		$http.get(url,{
			headers:{
				"Content-type":"application/x-www-form-urlencoded;charset=UTF-8"
			}
	...
	*/
	$scope.queryHospital = function(){
		var url = Tool.getSession("host")+"/wx/hospital/querybyid?id="+$scope.hosId;
		$http.get(url,{
			headers:{
				"Content-type":"application/x-www-form-urlencoded;charset=UTF-8"
			}
	...
	*/
	$scope.queryHospitalImg = function(){
		var url = Tool.getSession("host")+"/wx/image/querybymainid?type=HOSPITAL&mainId="+$scope.hosId;
		$http.get(url,{
			headers:{
				"Content-type":"application/x-www-form-urlencoded;charset=UTF-8"
			}
	...
	*/
	$scope.queryDoctor = function(){
		var url=Tool.getSession("host")+"/wx/doctor/queryscheduledoctorbyproductid";
		var params = "productId="+$scope.proId;
		$http.post(url,params,{
			headers:{
				"Content-type":"application/x-www-form-urlencoded;charset=UTF-8"
	...
	*/
	$scope.queryDoctorSchedule = function(docId){
		if($scope.hasDocSchedule==docId){
			...
			$scope.loading = true;
			var url = Tool.getSession("host")+"/wx/schedule/querybydoctorid?doctorId="+docId;
			$http.get(url,{
				headers:{
					"Content-type":"application/x-www-form-urlencoded;charset=UTF-8"
				}
	...
	*/
	$scope.orderDetail = function(){
		if(!$scope.noSelectTime){
			if(!Tool.isLogin()){
				Tool.alert($scope,"请先登录并完善个人信息后才能下单!",function(){
					Tool.goPage("/new/htmls/login.html");
				})
			}else if(!Tool.isUserInfoComplete()){
				Tool.alert($scope,"请完善个人信息后才能下单!",function(){
					Tool.goPage("/new/htmls/user.html");
				})
			}else{
				var url = Tool.getSession("host")+"/wx/order/checkCodeMoney";
				if($scope.order.code&&$scope.order.discountid){
					var params = {
						"productId":$scope.order.productId,
						"code":$scope.order.code,
						"discountid":$scope.order.discountid
					}
				}else{
					var params = {
						"productId":$scope.order.productId
				...
				$http.post(url,params,{
					...
					Tool.setSession("order",$scope.order);
